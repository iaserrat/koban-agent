...
/// A single configurable heuristic: when an item on `surface` changes in a way listed in
/// `triggers` and satisfies `match`, the engine raises a `Finding` of `severity`. Rules are
/// data - the built-in set is just the default configuration (see CLAUDE.md).
struct HeuristicRule: Hashable, Identifiable {
    var id: String
    var surface: MonitoredSurface
    var enabled: Bool
    var triggers: [ChangeKind]
    var match: RuleMatch
...
extension HeuristicRule: Decodable {
    ...
    private static let defaultTriggers: [ChangeKind] = [.added, .modified]
    ...
    init(from decoder: any Decoder) throws {
        ...
        triggers = try container.decodeIfPresent([ChangeKind].self, forKey: .triggers) ?? Self.defaultTriggers
