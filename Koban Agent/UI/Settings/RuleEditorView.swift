...
struct RuleEditorView: View {
        ...
        SettingsRow(label: "Triggers") {
            HStack(spacing: Metrics.spacingSmall) {
                ForEach(RuleTrigger.allCases) { trigger in
                    Toggle(trigger.displayName, isOn: triggerBinding(trigger))
                        .toggleStyle(.button)
                        .tint(Palette.accent)
                }
            }
    ...
    private func triggerBinding(_ trigger: RuleTrigger) -> Binding<Bool> {
        Binding(
            get: { rule.triggers.contains(trigger) },
            set: { isOn in
                var selected = Set(rule.triggers)
                if isOn { selected.insert(trigger) } else { selected.remove(trigger) }
                rule.triggers = RuleTrigger.allCases.filter { selected.contains($0) }
            }
        )
    }
}
