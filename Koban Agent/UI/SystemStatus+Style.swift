...
/// Maps the home dashboard's `SystemStatus` verdict to its hero treatment, so colour, glyph, and
...
extension SystemStatus {
    ...
    /// The shield-family glyph for the emblem: a calm check when clear, a plain shield while
    ...
    /// Whether the emblem breathes. Only the in-progress states (live and starting) pulse; a problem
    /// holds still so the alarm reads as steady, not animated.
    var isLive: Bool {
        switch self {
