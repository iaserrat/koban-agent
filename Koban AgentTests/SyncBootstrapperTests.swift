...
struct SyncBootstrapperTests {
    ...
    @Test
    func existingEnrollmentHydratesSyncIdentityWhenRemoteConfigIsUnchanged() async throws {
        try await Fixture.withTemporaryDirectory { directory in
            let stateURL = directory.appending(component: SensorProtocolConstants.enrollmentStateFileName)
            try EnrollmentStateStore(fileURL: stateURL).save(enrollmentState())
            let transport = try QueueTransport(responses: [
                JSONEncoder().encode(emptyConfigResponse())
            ])
            let bootstrapper = SyncBootstrapper(
                client: SyncHTTPClient(transport: transport),
                stateStore: EnrollmentStateStore(fileURL: stateURL),
            ...
            #expect(bootstrapped.sync.endpoint == "https://fleet.example.com")
            #expect(bootstrapped.sync.sensorToken == "sensor-token")
            let defaultPollIntervalSeconds = DefaultConfiguration.value.watch.pollIntervalSeconds
            #expect(bootstrapped.watch.pollIntervalSeconds == defaultPollIntervalSeconds)

            let paths = await transport.paths
            #expect(paths == ["/api/sensor/v1/config"])
        }
    }
    ...
    private func enrollmentState() -> EnrollmentState {
        EnrollmentState(
            ...
            clientCertificate: Data("certificate".utf8),
            certificateExpiresAt: "2026-06-01T10:00:00Z",
            configGeneration: Self.generation
        )
    }
    ...
    private func emptyConfigResponse() -> GetConfigResponse {
        GetConfigResponse(
            generation: Self.generation,
            configJSON: Data(),
            signature: Data()
